...
class PrivacyMonitor:
    ...
    def format_output(self, status: Dict) -> Dict:
        ...
        if location['is_active'] and location['active_access']:
            ...
            # Only show if location services are available - don't clutter by default
            pass
...
def main():
    if len(sys.argv) > 1:
        ...
        if command == "toggle-webcam":
            # Smart webcam toggle with feedback
            try:
                monitor = PrivacyMonitor()
                
                print("🔍 Checking current webcam status...")
                ...
                if webcam_status['devices_in_use']:
                    ...
                    print("📹 Camera in use - killing processes...")
                    killed_processes = []
                    ...
                    # Camera is disabled - try to enable it
                    print(f"📵 Camera disabled - attempting to enable {camera_module}...")
                    try:
                        result = subprocess.run(['/usr/bin/sudo', '-n', '/sbin/modprobe', camera_module], 
                                              capture_output=True, text=True, timeout=10)
                        if result.returncode == 0:
                            message = f"Camera hardware enabled ({camera_module})"
                            print(f"✅ modprobe {camera_module} succeeded")
                        else:
                            message = f"Camera enable failed - need passwordless sudo setup"
                            print(f"❌ modprobe failed: {result.stderr}")
                    except Exception as e:
                        message = f"Camera enable failed - permission issue"
                        print(f"❌ Exception: {e}")
                    
                elif webcam_status['has_webcam']:
                    # Camera available but not in use - disable hardware
                    print(f"📷 Camera available - attempting to disable {camera_module}...")
                    try:
                        result = subprocess.run(['/usr/bin/sudo', '-n', '/sbin/modprobe', '-r', camera_module], 
                                              capture_output=True, text=True, timeout=10)
                        if result.returncode == 0:
                            message = f"Camera hardware disabled ({camera_module})"
                            print(f"✅ modprobe -r {camera_module} succeeded")
                        else:
                            message = f"Camera disable failed - need passwordless sudo setup"
                            print(f"❌ modprobe -r failed: {result.stderr}")
                    except Exception as e:
                        message = f"Camera disable failed - permission issue"
                        print(f"❌ Exception: {e}")
                else:
                ...
                # Refresh waybar to show updated status
                try:
                    subprocess.run(['pkill', '-RTMIN+10', 'waybar'], capture_output=True, timeout=2)
                except:
                    pass
                
            except Exception as e:
                error_msg = f"Camera toggle failed: {str(e)}"
                print(f"❌ Exception in toggle: {e}")
                try:
            ...
            if location['is_active']:
                ...
                print("🗺️ LOCATION: No services detected")
