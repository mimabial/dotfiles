...
class DenvConfig:
    ...
    def _load_config(self):
        """Load configuration from $XDG_STATE_HOME/denv/config"""
        state_dir = os.path.expanduser(os.getenv("XDG_STATE_HOME", "~/.local/state"))
        config_file = os.path.join(state_dir, "denv", "config")
    ...
    def get_value(self, key, default=None):
        """Get value from config, falling back to environment, then default"""
...
class CavaServer:
    ...
    def __init__(self):
        ...
        self.socket_file = os.path.join(self.runtime_dir, "denv", "cava.sock")
        self.pid_file = os.path.join(self.runtime_dir, "denv", "cava.pid")
        self.temp_dir = Path(os.path.join(self.runtime_dir, "denv"))
    ...
    def _reload_cava_process(self):
        ...
        denv_config = DenvConfig()
        bars = int(denv_config.get_value("CAVA_BARS", 16))
        range_val = int(denv_config.get_value("CAVA_RANGE", 15))
        channels = denv_config.get_value("CAVA_CHANNELS", "stereo")
        reverse = denv_config.get_value("CAVA_REVERSE", 0)
        try:
            reverse = int(reverse)
        except Exception:
    ...
    def _create_cava_config(self, bars=16, range_val=15, channels="stereo", reverse=0, prefix=""): 
        """Create cava configuration file with channels and reverse support, using DenvConfig with or without prefix as appropriate"""
        denv_config = DenvConfig()

        if prefix:
            config_channels = denv_config.get_value(f"CAVA_{prefix}_CHANNELS")
            config_reverse = denv_config.get_value(f"CAVA_{prefix}_REVERSE")
        else:
            config_channels = denv_config.get_value("CAVA_CHANNELS")
            config_reverse = denv_config.get_value("CAVA_REVERSE")
...
class CavaClient:
    ...
    @staticmethod
    def parse_command_config(denv_config, command, args):
        ...
        if hasattr(args, "bar_array") and args.bar_array:
            ...
            bar_array = denv_config.get_value(f"{prefix}_BAR_ARRAY")
            if bar_array and isinstance(bar_array, list):
                bar_chars = bar_array
            else:
                bar_chars = args.bar or denv_config.get_value(f"{prefix}_BAR", "▁▂▃▄▅▆▇█")
        ...
        width = (
            ...
            else int(denv_config.get_value(f"{prefix}_WIDTH", "0") or 0)
        )
        if not width:
        ...
        if args.stb is not None:
            ...
            standby_mode = denv_config.get_value(f"{prefix}_STANDBY", "0")
