...
Wall_Cache() {
    ...
    if [ "${set_as_global}" == "true" ]; then
        ...
        "${LIB_DIR}/denv/swwwallcache.sh" -w "${wallList[setIndex]}" &>/dev/null
        "${LIB_DIR}/denv/color.set.sh" "${wallList[setIndex]}" &
...
main() {
    ...
    if [ -f "${LIB_DIR}/denv/wallpaper.${wallpaper_backend}.sh" ] && [ -n "${wallpaper_backend}" ]; then
        print_log -sec "wallpaper" "Using backend: ${wallpaper_backend}"
        "${LIB_DIR}/denv/wallpaper.${wallpaper_backend}.sh" "${wallSet}"
    else
...
while true; do
    ...
    -j | --json)
        ...
        ;;
    -S | --select)
        "${LIB_DIR}/denv/swwwallcache.sh" w &>/dev/null &
