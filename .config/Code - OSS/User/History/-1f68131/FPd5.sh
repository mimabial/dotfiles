...
HYDRA_LOG="$(date +'%y%m%d_%Hh%Mm%Ss')"
export flg_DryRun flg_Nvidia flg_Shell flg_Install flg_ThemeInstall HYDRA_LOG
...
EOF
    ...
    trap 'mv "${scrDir}/install_pkg.lst" "${cacheDir}/logs/${HYDRA_LOG}/install_pkg.lst"' EXIT
    ...
    if ! chk_list "aurhlpr" "${aurList[@]}"; then
        ...
        *)
        ...
        if [[ -z "$getAur" ]]; then
            print_log -sec "AUR" -crit "No AUR helper found..." "Log file at ${cacheDir}/logs/${HYDRA_LOG}"
            exit 1
        fi
    fi
    ...
    if ! chk_list "myShell" "${shlList[@]}"; then
        ...
        *)
        ...
        if [[ -z "$myShell" ]]; then
            print_log -sec "shell" -crit "No shell found..." "Log file at ${cacheDir}/logs/${HYDRA_LOG}"
            exit 1
        else
    ...
    if ! grep -q "^#user packages" "${scrDir}/install_pkg.lst"; then
        print_log -sec "pkg" -crit "No user packages found..." "Log file at ${cacheDir}/logs/${HYDRA_LOG}/install.sh"
        exit 1
    fi
...
EOF
    ...
    if [ "${flg_DryRun}" -ne 1 ]; then
        "$HOME/.local/lib/hydra/swwwallcache.sh" -t ""
        "$HOME/.local/lib/hydra/theme.switch.sh" -q || true
...
print_log -stat "Log" "View logs at ${cacheDir}/logs/${HYDRA_LOG}"
if [ $flg_Install -eq 1 ] ||
    [ $flg_Restore -eq 1 ] ||
