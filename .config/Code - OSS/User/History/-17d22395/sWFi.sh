...
if pkg_installed grub && [ -f /boot/grub/grub.cfg ]; then
    ...
    if [ ! -f /etc/default/grub.hydra.bkp ] && [ ! -f /boot/grub/grub.hydra.bkp ]; then
        print_log -g "[bootloader] " -b "configure :: " "grub"
        sudo cp /etc/default/grub /etc/default/grub.hydra.bkp
        sudo cp /boot/grub/grub.cfg /boot/grub/grub.hydra.bkp
...
if pkg_installed systemd && nvidia_detect && [ "$(bootctl status 2>/dev/null | awk '{if ($1 == "Product:") print $2}')" == "systemd-boot" ]; then
    ...
    if [ "$(find /boot/loader/entries/ -type f -name '*.conf.hydra.bkp' 2>/dev/null | wc -l)" -ne "$(find /boot/loader/entries/ -type f -name '*.conf' 2>/dev/null | wc -l)" ]; then
        ...
        find /boot/loader/entries/ -type f -name "*.conf" | while read -r imgconf; do
            sudo cp "${imgconf}" "${imgconf}.hydra.bkp"
...
if [ -f /etc/pacman.conf ] && [ ! -f /etc/pacman.conf.hydra.bkp ]; then
    print_log -g "[PACMAN] " -b "modify :: " "adding extra spice to pacman..."

    # shellcheck disable=SC2154
    [ "${flg_DryRun}" -eq 1 ] || sudo cp /etc/pacman.conf /etc/pacman.conf.hydra.bkp
