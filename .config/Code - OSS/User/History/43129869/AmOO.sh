...
scrDir=${scrDir:-$HOME/.local/lib/hydra}
confDir="${confDir:-$XDG_CONFIG_HOME}"
cacheDir="${HYDRA_CACHE_HOME:-"${XDG_CACHE_HOME}/hydra"}"
...
fn_background() {
    ...
    if [ "${is_video}" -eq 1 ]; then
        ...
        mkdir -p "${HYDRA_CACHE_HOME}/wallpapers/thumbnails"
        cached_thumb="$HYDRA_CACHE_HOME/wallpapers/$(${hashMech:-sha1sum} "${WP}" | cut -d' ' -f1).png"
...
fn_profile() {
    ...
    if [ -f "$HOME/.face.icon" ]; then
        ...
        cp "$XDG_DATA_HOME/icons/Wallbash-Icon/hydra.png" "${profilePath}.png"
    fi
    return 0
}
...
fn_mpris() {
    ...
    if [[ "${player_status}" == "Playing" ]]; then
        ...
        if [ -f "$HOME/.face.icon" ]; then
            ...
            if ! cmp -s "$XDG_DATA_HOME/icons/Wallbash-Icon/hydra.png" "${THUMB}.png"; then
                cp "$XDG_DATA_HOME/icons/Wallbash-Icon/hydra.png" "${THUMB}.png"
                pkill -USR2 hyprlock >/dev/null 2>&1 # updates the mpris thumbnail
            fi
        fi
        exit 1
...
fn_cava() {
    ...
    config_file="$HYDRA_RUNTIME_DIR/cava.hyprlock"
...
generate_conf() {
    ...
    local hydra_hyprlock_conf=${SHARE_DIR:-$XDG_DATA_HOME}/hydra/hyprlock.conf
...
# If a persistent layout path is declared in \$XDG_CONFIG_HOME/hypr/hydra.conf,
# the above layout setting will be ignored.
# this should be the full path to the layout file.

#*  All boilerplate configurations are handled by Hydra 
source = ${hydra_hyprlock_conf}
...
# Alternatively, you can statically source the layout in '~/.config/hypr/hydra.conf'.
...
# Hydra also provides custom variables that extend the functionality of Hyprlock.
...
# - if available, otherwise, it will show the Hydra logo.
...
# - Hydra will run 'font.sh resolve' to install the font for you.
...
if [ -z "${*}" ]; then
    if [ ! -f "$HYDRA_CACHE_HOME/wallpapers/hyprlock.png" ]; then
        print_log -sec "hyprlock" -stat "setting" " $HYDRA_CACHE_HOME/wallpapers/hyprlock.png"
        "${scrDir}/wallpaper.sh" -s "$(readlink "${HYDRA_THEME_DIR}/wall.set")" --backend hyprlock
    fi
    hyprlock
fi
