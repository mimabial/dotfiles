...
# add overrides in [wallpaper.swww] in ~/.config/hydra/config.toml
...
selected_wall="${1:-"$$HYDRA_CACHE_HOME/wall.set"}"
lockFile="$HYDRA_RUNTIME_DIR/$(basename "${0}").lock"
if [ -e "${lockFile}" ]; then
    cat <<EOF
...
if [ "${is_video}" -eq 1 ]; then
    ...
    cached_thumb="$HYDRA_CACHE_HOME/wallpapers/$(${hashMech:-sha1sum} "${selected_wall}" | cut -d' ' -f1).png"
