...
kmenuDesk="${kmenuPath}/hydrawallpaper.desktop"
tgtPath="$(dirname "${HYDRA_THEME_DIR}")"
get_themes
...
if [ ! -z "${setTheme}" ] && [ ! -z "${setWall}" ]; then
    ...
    if [[ "${wallHash[@]}" == *"${inwallHash}"* ]]; then
        notify-send -a "Hydra Notify" -i "${thmbDir}/${inwallHash}.sqre" "Error" "Hash matched in ${setTheme}"
        exit 0
    fi
    ...
    notify-send -a "Hydra Alert" -i "${thmbDir}/${inwallHash}.sqre" "Wallpaper set in ${setTheme}"
