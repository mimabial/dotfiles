...
# ! export XDG_DATA_DIRS="$XDG_DATA_HOME/hydra:/usr/local/share/hydra/:/usr/share/hydra/:$XDG_DATA_DIRS" # Causes issues https://github.com/Hydra-Project/Hydra/issues/308#issuecomment-2691229673

# hydra envs
export HYDRA_CONFIG_HOME="${XDG_CONFIG_HOME}/hydra"
export HYDRA_DATA_HOME="${XDG_DATA_HOME}/hydra"
export HYDRA_CACHE_HOME="${XDG_CACHE_HOME}/hydra"
export HYDRA_STATE_HOME="${XDG_STATE_HOME}/hydra"
export HYDRA_RUNTIME_DIR="${XDG_RUNTIME_DIR}/hydra"
export ICONS_DIR="${XDG_DATA_HOME}/icons"
export FONTS_DIR="${XDG_DATA_HOME}/fonts"
export THEMES_DIR="${XDG_DATA_HOME}/themes"

#legacy hydra envs // should be deprecated

export confDir="${XDG_CONFIG_HOME:-$HOME/.config}"
export hydraConfDir="$HYDRA_CONFIG_HOME"
export cacheDir="$HYDRA_CACHE_HOME"
export thmbDir="$HYDRA_CACHE_HOME/thumbs"
export dcolDir="$HYDRA_CACHE_HOME/dcols"
...
get_hashmap() {
    ...
    for wallSource in "$@"; do
        ...
        if [ -z "${hashMap}" ]; then
            notify-send -a "Hydra Alert" "WARNING: No compatible wallpapers found in \"${wallSource}\""
            continue
        fi
...
get_themes() {
    ...
    done < <(find "${hydraConfDir}/themes" -mindepth 1 -maxdepth 1 -type d)
...
[ -f "${HYDRA_RUNTIME_DIR}/environment" ] && source "${HYDRA_RUNTIME_DIR}/environment"
[ -f "$HYDRA_STATE_HOME/staterc" ] && source "$HYDRA_STATE_HOME/staterc"
...
if [ -z "${HYDRA_THEME}" ] || [ ! -d "${hydraConfDir}/themes/${HYDRA_THEME}" ]; then
    get_themes
    HYDRA_THEME="${thmList[0]}"
fi

HYDRA_THEME_DIR="${hydraConfDir}/themes/${HYDRA_THEME}"
wallbashDirs=(
    "${hydraConfDir}/wallbash"
    "${XDG_DATA_HOME}/hydra/wallbash"
    "/usr/local/share/hydra/wallbash"
    "/usr/share/hydra/wallbash"
)
...
pkg_installed() {
    ...
    if command -v "${pkgIn}" &>/dev/null; then
        ...
        return 0
    elif hydra-shell pm.sh pq "${pkgIn}" &>/dev/null; then
        return 0
    else
        return 1
    fi
...
set_conf() {
    local varName="${1}"
    local varData="${2}"
    touch "${XDG_STATE_HOME}/hydra/staterc"

    if [ "$(grep -c "^${varName}=" "${XDG_STATE_HOME}/hydra/staterc")" -eq 1 ]; then
        sed -i "/^${varName}=/c${varName}=\"${varData}\"" "${XDG_STATE_HOME}/hydra/staterc"
    else
        echo "${varName}=\"${varData}\"" >>"${XDG_STATE_HOME}/hydra/staterc"
    fi
}
...
check_package() {
    ...
    local lock_file="${XDG_RUNTIME_DIR:-/tmp}/hydra/__package.lock"
    mkdir -p "${XDG_RUNTIME_DIR:-/tmp}/hydra"
...
get_hyprConf() {
    local hyVar="${1}"
    local file="${2:-"$HYDRA_THEME_DIR/hypr.theme"}"
    local gsVal
    ...
    if [ -z "${gsVal}" ] || [[ "${gsVal}" == \$* ]]; then
        ...
        *)
            grep "^[[:space:]]*\$default.${hyVar}\s*=" \
                "XDG_DATA_HOME/hydra/hydra.conf" \
                "$XDG_DATA_HOME/hydra/hyprland.conf" \
                "/usr/local/share/hydra/hydra.conf" \
                "/usr/local/share/hydra/hyprland.conf" \
                "/usr/share/hydra/hydra.conf" \
                "/usr/share/hydra/hyprland.conf" 2>/dev/null |
...
paste_string() {
    ...
    ignore_paste_file="$HYDRA_STATE_HOME/ignore.paste"
...
accepted_mime_types() {
    ...
    for mime_type in "${mime_types_array[@]}"; do
        if file --mime-type -b "${file}" | grep -q "^${mime_type}"; then
            ...
            notify-send -u critical -a "Hydra-Alert" "File type not supported for this wallpaper backend."
        fi
