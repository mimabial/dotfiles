...
image_dirs=()
hydra_distro_logo=${iconDir}/Wallbash-Icon/distro/$LOGO
...
logo) # eats around 13 ms
  random() {
    (
      ...
      if [ -n "${HYDRA_THEME}" ] && [ -d "${confDir}/hydra/themes/${HYDRA_THEME}/logo" ]; then
        image_dirs+=("${confDir}/hydra/themes/${HYDRA_THEME}/logo")
      fi
      [ -d "$HYDRA_CACHE_HOME" ] && image_dirs+=("$HYDRA_CACHE_HOME")
      [ -f "$hydra_distro_logo" ] && echo "${hydra_distro_logo}"
...
options:
  ...
  --theme   Display a logo inside the hydra theme directory
...
HELP
  ...
  (
    for arg in "$@"; do
      case $arg in
      --quad)
        echo "$HYDRA_CACHE_HOME/wall.quad"
        ;;
      --sqre)
        echo "$HYDRA_CACHE_HOME/wall.sqre"
        ;;
      --prof)
        [ -f "$HOME/.face.icon" ] && echo "$HOME/.face.icon"
        ;;
      --os)
        echo "$hydra_distro_logo"
        ;;
      --local)
        image_dirs+=("${confDir}/fastfetch/logo")
        ;;
