...
WALLBASH_DIRS=(
    "${XDG_CONFIG_HOME:-$HOME.config}/hydra/wallbash"
    "${XDG_DATA_HOME:-$HOME/.local/share}/hydra/wallbash"
...
if [ -d "$2" ]; then
    ...
    git_theme=${git_path#*/}
    branch_dir=${branch//\//_}
    cache_dir="${XDG_CACHE_HOME:-"$HOME/.cache"}/hydra"
...
FAV_THEME_DIR="${THEME_DIR}/Configs/.config/hydra/themes/${THEME_NAME}"
[ ! -d "${FAV_THEME_DIR}" ] && print_log -r "[ERROR] " "'${FAV_THEME_DIR}'" -y " Do not Exist" && exit 1

# config=$(find "${dcolDir}" -type f -name "*.dcol" | awk -v favTheme="${THEME_NAME}" -F 'theme/' '{gsub(/\.dcol$/, ".theme"); print ".config/hydra/themes/" favTheme "/" $2}')
config=$(find "${WALLBASH_DIRS[@]}" -type f -path "*/theme*" -name "*.dcol" 2>/dev/null | awk '!seen[substr($0, match($0, /[^/]+$/))]++' | awk -v favTheme="${THEME_NAME}" -F 'theme/' '{gsub(/\.dcol$/, ".theme"); print ".config/hydra/themes/" favTheme "/" $2}')
restore_list=""
...
if [ -f "${FAV_THEME_DIR}/theme.dcol" ]; then
    ...
    restore_list+="Y|Y|\${HOME}/.config/hydra/themes/${THEME_NAME}|theme.dcol|hyprland\n"
fi
...
# populate wallpaper
theme_wallpapers="${confDir}/hydra/themes/${THEME_NAME}/wallpapers"
...
# populate logos
theme_logos="${confDir}/hydra/themes/${THEME_NAME}/logo"
if [ -n "${logos}" ]; then
