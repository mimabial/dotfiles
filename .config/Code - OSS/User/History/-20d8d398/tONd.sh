#!/usr/bin/env bash
#|---/ /+-------------------------------+---/ /|#
#|--/ /-| Script to remove Hydra configs |--/ /-|#
#|-/ /--| Prasanth Rangan               |-/ /--|#
#|/ /---+-------------------------------+/ /---|#
...
[ -d "$HOME/.config/hydra" ] && rm -rf "$HOME/.config/hydra"
[ -d "$HOME/.cache/hydra" ] && rm -rf "$HOME/.cache/hydra"
[ -d "$HOME/.local/state/hydra" ] && rm -rf "$HOME/.local/state/hydra"
...
Remove Hydra related backups/icons/fonts/themes manually from these paths
