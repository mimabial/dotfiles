...
[ -z "${HYDRA_THEME}" ] && echo "ERROR: unable to detect theme" && exit 1
get_themes
...
[[ ! " ${thmList[*]} " =~ " ${themeSet} " ]] && themeSet="${HYDRA_THEME}"

set_conf "HYDRA_THEME" "${themeSet}"
...
#// hypr
# shellcheck disable=SC2154
sed '1d' "${HYDRA_THEME_DIR}/hypr.theme" >"${confDir}/hypr/themes/theme.conf" # Useless and already handled by swwwallbash.sh but kept for robustness
# shellcheck disable=SC2154
...
export scrDir
find "$HYDRA_CACHE_HOME/wallpapers" -name "*.png" -exec sh -c '
    for file; do
...
if [ "$quiet" = true ]; then
    "${scrDir}/wallpaper.sh" -s "$(readlink "${HYDRA_THEME_DIR}/wall.set")" --global >/dev/null 2>&1
else
    "${scrDir}/wallpaper.sh" -s "$(readlink "${HYDRA_THEME_DIR}/wall.set")" --global
fi
