...
wallbashDirs=(
    "$HOME/.config/hydra/wallbash"
    "$HOME/.local/share/hydra/wallbash"
...
if [ -d "$2" ]; then
    ...
    Git_Owner=${Git_Path%/*}
    branch_dir=${branch//\//_}
    cacheDir=${cacheDir:-"$HOME/.cache/hydra"}
...
Fav_Theme_Dir="${Theme_Dir}/Configs/.config/hydra/themes/${Fav_Theme}"
[ ! -d "${Fav_Theme_Dir}" ] && print_prompt -r "[ERROR] " "'${Fav_Theme_Dir}'" -y " Do not Exist" && exit 1

# config=$(find "${dcolDir}" -type f -name "*.dcol" | awk -v favTheme="${Fav_Theme}" -F 'theme/' '{gsub(/\.dcol$/, ".theme"); print ".config/hydra/themes/" favTheme "/" $2}')
config=$(find "${wallbashDirs[@]}" -type f -path "*/theme*" -name "*.dcol" 2>/dev/null | awk '!seen[substr($0, match($0, /[^/]+$/))]++' | awk -v favTheme="${Fav_Theme}" -F 'theme/' '{gsub(/\.dcol$/, ".theme"); print ".config/hydra/themes/" favTheme "/" $2}')
restore_list=""
...
if [ -f "${Fav_Theme_Dir}/theme.dcol" ]; then
    ...
    restore_list+="Y|Y|\${HOME}/.config/hydra/themes/${Fav_Theme}|theme.dcol|hyprland\n"
fi
...
# populate wallpaper
Fav_Theme_Walls="${confDir}/hydra/themes/${Fav_Theme}/wallpapers"
...
# populate logos
Fav_Theme_Logos="${confDir}/hydra/themes/${Fav_Theme}/logo"
if [ -n "${logos}" ]; then
...
if [ "${3}" != "--skipcaching" ]; then
    "$HOME/.local/lib/hydra/swwwallcache.sh" -t "${Fav_Theme}"
    "$HOME/.local/lib/hydra/theme.switch.sh"
fi
