...
[ -d "${HYDRA_THEME_DIR}" ] && cacheIn="${HYDRA_THEME_DIR}" || exit 1
...
fn_wallcache() {
    ...
    if [ "${is_video}" -eq 1 ]; then
        ...
        then
            ...
            notify-send -a "Hydra wallpaper" "Extracting thumbnail from video wallpaper..."
...
# Function to cache any links that are hydra related
fn_envar_cache() {
    if command -v rofi &>/dev/null; then
        if [[ ! "$XDG_DATA_DIRS" =~ share/hydra ]]; then
            mkdir -p "$XDG_DATA_HOME/rofi/themes"
            ln -snf "$XDG_DATA_HOME/hydra/rofi/themes"/* "$XDG_DATA_HOME/rofi/themes/"
        fi
    fi
}
...
while getopts "w:t:f" option; do
    ...
    t) # generate cache for input theme
        cacheIn="$(dirname "${HYDRA_THEME_DIR}")/${OPTARG}"
        if [ ! -d "${cacheIn}" ]; then
    ...
    f) # full cache rebuild
        cacheIn="$(dirname "${HYDRA_THEME_DIR}")"
        mode="_force"
        ;;
    *) # invalid option
        echo "... invalid option ..."
