...
function command_not_found_handler {
    ...
    if [ ! command -v "${PM}" ] &>/dev/null; then
        for path in "/usr/lib/hydra" "/usr/local/lib/hydra" "$HOME/.local/lib/hydra" "$HOME/.local/bin"; do
    ...
    if ! command -v "${PM}" &>/dev/null; then
        printf "${bright}${red}We cannot find package manager script (${purple}pm.sh${red}) from ${green}Hydra${reset}\n"
        return 127
    fi
...
function load_zsh_plugins {
    ...
    # Load Plugins
    hydra_plugins=(git zsh-256color zsh-autosuggestions zsh-syntax-highlighting)
    plugins+=("${plugins[@]}" "${hydra_plugins[@]}" git zsh-256color zsh-autosuggestions zsh-syntax-highlighting)
    # Deduplicate plugins
    plugins=("${plugins[@]}")
    plugins=($(printf "%s\n" "${plugins[@]}" | sort -u))
...
function slow_load_warning {
    local lock_file="/tmp/.hydra_slow_load_warning.lock"
    ...
    ⚠️ Warning: Shell startup took more than ${time_limit} seconds. Consider optimizing your configuration.
        ...
        2. Duplicate plugins initialization.
            ...
            - Hydra already sources the oh-my-zsh.sh file for you.
            - It is important to remove all Hydra related
                configurations from your .zshrc file as Hydra will handle it for you.
            - Check the '.zshrc' file from the repo for a clean configuration.
                https://github.com/Hydra-Project/Hydra/blob/master/Configs/.zshrc
        3. Check the '~/.hydra.zshrc' file for any slow initialization scripts.
    ...
    For more information, on the possible causes of slow shell startup, see:
        🌐 https://github.com/Hydra-Project/Hydra/wiki
...
function cleanup {
    rm -f /tmp/.hydra_slow_load_warning.lock
}
...
if [ -t 1 ]; then
    ...
    [[ -f ~/.hydra.zshrc ]] && source ~/.hydra.zshrc
