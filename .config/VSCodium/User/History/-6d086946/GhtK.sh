...
if [ "${enableWallDcol}" -eq 0 ]; then
    HYDRA_THEME_DIR="${HYDRA_THEME_DIR:-$confDir/hydra/themes/$HYDRA_THEME}"
    dcol_mode=$(get_hyprConf "COLOR_SCHEME")
    dcol_mode=${dcol_mode#prefer-}
    # shellcheck disable=SC1091
    [ -f "${HYDRA_THEME_DIR}/theme.dcol" ] && source "${HYDRA_THEME_DIR}/theme.dcol"
fi
