...
REPO_URL = "https://github.com/Hydra-Project/hydra-gallery.git"
CLONE_DIR = os.path.join(
    os.getenv("XDG_CACHE_HOME", os.path.expanduser("~/.cache")),
    "hydra/gallery-database",
)
JSON_DATA = None
...
def fetch_data():
    global JSON_DATA
    json_file_path = os.path.join(CLONE_DIR, "hydra-themes.json")
...
def fzf_menu():
    try:
        ...
        subprocess.run(["hydra-shell", "reload"], check=True)
...
def fetch_all_themes():
    ...
    themes_dir = os.path.join(config_home, "hydra/themes")
...
def main():
    parser = argparse.ArgumentParser(
        description="Imports themes from hydra-gallery repository",
        epilog="Env:\n"
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="THEME",
        help="Fetch and update a specific theme by name (`all` to fetch all themes located in 'xdg_config/hydra/themes')",
    )
