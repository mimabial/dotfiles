...
if ! source "$(command -v hydra-shell)"; then
...
take_screenshot() {
	...
	if "$LIB_DIR/hydra/grimblast" "${extra_args[@]}" copysave "$mode" "$temp_screenshot"; then
		if ! "${annotation_tool}" "${annotation_args[@]}"; then
			notify-send -a "Hydra Alert" "Screenshot Error" "Failed to open annotation tool"
			return 1
		fi
	else
		notify-send -a "Hydra Alert" "Screenshot Error" "Failed to take screenshot"
		return 1
	fi
}
...
sc) #? 󱉶 Use 'tesseract' to scan image then add to clipboard
	...
	if ! GEOM=$(slurp); then
		notify-send -a "Hydra Alert" "OCR preview: Invalid geometry" -e -i "dialog-error"
		exit 1
	fi
	...
	notify-send -a "Hydra Alert" "OCR preview" -i "${temp_screenshot}" -e
...
if [ -f "${save_dir}/${save_file}" ]; then
	notify-send -a "Hydra Alert" -i "${save_dir}/${save_file}" "saved in ${save_dir}"
fi
