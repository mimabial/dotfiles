...
for i in "${!codeVsix[@]}"; do
    ...
    if [ -z "${file}" ]; then
        [ -f "${cacheDir}/landing/Code_Wallbash.vsix" ] || curl -L -o "${cacheDir}/landing/Code_Wallbash.vsix" https://github.com/Hydra-Project/code-wallbash/raw/refs/heads/master/release/Code_Wallbash.vsix
        case ${codeVsix[i]} in
        *".cursor"*)
...
for i in "${!codeConf[@]}"; do
    ...
    extTheme="$(
        "$LIB_DIR/hydra/parse.json.py" -CRQ '.["workbench.colorTheme"]' "${codeConf[i]}/User/settings.json"
    )"
    if [ "${extTheme}" != "${set_theme}" ]; then
        "$LIB_DIR/hydra/parse.json.py" -CU '.["workbench.colorTheme"]' "${set_theme}" "${codeConf[i]}/User/settings.json"
