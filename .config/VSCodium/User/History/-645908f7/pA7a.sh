...
rofiStyleDir="${SHARE_DIR}/hydra/rofi/themes"
rofiAssetDir="${SHARE_DIR}/hydra/rofi/assets"
...
if [ -n "${RofiSel}" ]; then
    ...
    notify-send -a "Hydra Alert" -r 2 -t 2200 -i "${rofiAssetDir}/${RofiSel}.png" " style ${RofiSel} applied..."
fi
if [ -n "$ROFI_LAUNCH_STYLE" ]; then
    notify-send -a "Hydra Alert" -r 3 -u critical "Style: '$ROFI_LAUNCH_STYLE' is explicitly set, remove it in ~/.config/hydra/config.toml for changes to take effect."
fi
