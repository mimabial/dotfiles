...
landing_dir="${XDG_CACHE_HOME:-$HOME/.cache}/hydra/landing/fonts"
...
download_and_extract() {
    ...
    find "$temp_dir" -type f | while read -r file; do
        ...
        *.tar.xz)
            ...
            ;;
        *.ttf | *.otf)
            mkdir -p "${font_dir}/hydra"
            mv "$file" "${font_dir}/hydra/$name.ttf"
            echo "[font] $name installed successfully. Please restart hyprlock to apply changes."
            notify-send -i "preferences-desktop-font" "Hydra font" "${name} Intalled successfully"
            return 0
            ;;
        *)
        ...
        if ! cp -rn "${temp_dir}/${name}" "$font_dir"; then
            ...
            notify-send -i "preferences-desktop-font" "Hydra font" "Failed to extract $file"
            return 1
        fi
        notify-send -i "preferences-desktop-font" "Hydra font" "${name} Intalled successfully"
    done
