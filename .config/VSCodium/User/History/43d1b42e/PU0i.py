...
load_env_file(os.path.expanduser("~/.local/state/hydra/staterc"))
load_env_file(os.path.expanduser("~/.local/state/hydra/config"))
...
show_icon = os.getenv("WEATHER_SHOW_ICON", "False").lower() in (
    "true",
    "1",
    "t",
    "y",
...
if show_today_details:
    data["text"] = get_description(current_weather) + ' ' + data["text"]
if show_location:
