...
def get_venv_path():
    ...
    venv_path = os.path.join(xdg_base_dirs.xdg_state_home(), "hydra", "pip_env")
    if not os.path.exists(venv_path):
        venv_path = os.path.expanduser("~/.local/state/hydra/pip_env")
...
def create_venv(venv_path, requirements_file=None):
    ...
    if not os.path.exists(os.path.join(venv_path, "bin", "pip")):
        ...
        notify.send("Hydra PIP", "✅ Virtual environment created successfully")
    else:
        pass
...
def v_import(module_name):
    ...
    try:
        ...
        notify.send("Hydra PIP", f"Installing {module_name} module...")
        ...
        try:
            module = importlib.import_module(module_name)
            notify.send("Hydra PIP", f"Successfully installed {module_name}.")
            return module
...
def main(args):
    parser = argparse.ArgumentParser(description="Python environment manager for Hydra")
    ...
    requirements_file = os.path.join(
        os.path.expanduser("~/.local/lib/hydra/pyutils"), "requirements.txt"
    )
...
def hydra(args):
    """Python environment manager for Hydra.
