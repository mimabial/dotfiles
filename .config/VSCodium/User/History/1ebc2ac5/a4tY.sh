...
fn_steam() {
    ...
    if [ ! -z "$RofiSel" ]; then
        ...
        # dunstify "Hydra Alert" -a "Launching ${RofiSel}..." -i ${SteamThumb}/${launchid}_header.jpg -r 91190 -t 2200
        notify-send -a "Hydra Alert" -i "${SteamThumb}/${launchid}_header.jpg" "Launching ${RofiSel}..."
...
fn_lutris() {
    ...
    # [ ! -s "${meta_data}" ] &&
    notify-send -a "Hydra Alert" "Please wait... " -t 4000
...
EOF
    ...
    [ ! -s "${meta_data}" ] && notify-send -a "Hydra Alert" "Cannot Fetch Lutris Games!" && exit 1
    CHOICE=$(
    ...
    notify-send -a "Hydra Alert" -i "${icon_path}/${SLUG}.jpg" "Launching ${CHOICE}..."
...
if [ -z "${run_lutris}" ] || echo "$*" | grep -q "steam"; then
    ...
    if [ ! -f $SteamLib ] || [ ! -d $SteamThumb ]; then
        notify-send -a "Hydra Alert" "Steam library not found!"
        exit 1
    fi
    fn_steam
else
