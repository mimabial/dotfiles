...
selector_menu() {
    ...
    if [ -n "${RofiSel}" ]; then
        ...
        #// notify the user
        notify-send -a "Hydra Alert" -i "${rofiAssetDir}/theme_style_${selectedStyle}.png" "Style ${selectedStyle} applied..."
...
if [ -n "${rofiSel}" ]; then
    "${LIB_DIR}/hydra/themeswitch.sh" -s "${rofiSel}"
    # shellcheck disable=SC2154
    notify-send -a "Hydra Alert" -i "${iconsDir}/Wallbash-Icon/hydra.png" " ${rofiSel}"
fi
