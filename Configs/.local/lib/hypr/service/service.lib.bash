...
hypr_service_backup_root() {
  if [[ -z "${HYPR_SERVICE_BACKUP_ROOT:-}" ]]; then
    ...
    HYPR_SERVICE_BACKUP_ROOT="${XDG_CONFIG_HOME:-$HOME/.config}/cfg_backups/$(date +%Y%m%d_%H%M%S)_${label}"
  fi
  printf '%s\n' "${HYPR_SERVICE_BACKUP_ROOT}"
}
...
hypr_service_maybe_report_backup_root() {
  ...
  [[ -n "${first_entry}" ]] && printf 'Backups: %s\n' "${backup_root}"
}
...
hypr_service_parse_mode_cli() {
  local usage_fn="$1"
  local mode_name="$2"
  local forwarded_name="$3"
  shift 3
  local -n mode_ref="${mode_name}"
  local -n forwarded_ref="${forwarded_name}"
...
hypr_service_validate_mode() {
  case "${1:-}" in
    refresh | restore) return 0 ;;
    *) return 1 ;;
  esac
}
...
hypr_service_layer_source_path() {
  local layer="$1"
  local rel_path="$2"
  local default_root="${XDG_DATA_HOME:-$HOME/.local/share}/hypr/default"
