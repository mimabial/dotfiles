...
class HyprConfig:
    """Handle Hyprland configuration loading and parsing"""

    def __init__(self):
        self.config = self._load_config()

    def _load_config(self):
        """Load configuration from $XDG_STATE_HOME/hypr/config"""
        state_dir = os.path.expanduser(os.getenv("XDG_STATE_HOME", "~/.local/state"))
        config_file = os.path.join(state_dir, "hypr", "config")
        ...
        try:
            ...
            print(f"Warning: Could not load Hyprland config: {e}", file=sys.stderr)

        return config

    def get_value(self, key, default=None):
        """Get value from Hyprland config, falling back to environment, then default"""
...
class CavaServer:
    ...
    def __init__(self):
        ...
        self.socket_file = os.path.join(self.runtime_dir, "hypr", "cava.sock")
        self.pid_file = os.path.join(self.runtime_dir, "hypr", "cava.pid")
        self.temp_dir = Path(os.path.join(self.runtime_dir, "hypr"))
    ...
    def _reload_cava_process(self):
        ...
        bars = int(hypr_config.get_value("CAVA_BARS", 16))
        range_val = int(hypr_config.get_value("CAVA_RANGE", 15))
        channels = hypr_config.get_value("CAVA_CHANNELS", "stereo")
        reverse = hypr_config.get_value("CAVA_REVERSE", 0)
        try:
            reverse = int(reverse)
        except Exception:
    ...
    def _create_cava_config(
        ...
        """Create cava configuration file with channels and reverse support, using HyprConfig with or without prefix as appropriate"""
        hypr_config = HyprConfig()

        if prefix:
            config_channels = hypr_config.get_value(f"CAVA_{prefix}_CHANNELS")
            config_reverse = hypr_config.get_value(f"CAVA_{prefix}_REVERSE")
        else:
            config_channels = hypr_config.get_value("CAVA_CHANNELS")
            config_reverse = hypr_config.get_value("CAVA_REVERSE")
...
class CavaClient:
    ...
    @staticmethod
    def parse_command_config(hypr_config, command, args):
        ...
        if hasattr(args, "bar_array") and args.bar_array:
            ...
            bar_array = hypr_config.get_value(f"{prefix}_BAR_ARRAY")
            if bar_array and isinstance(bar_array, list):
                bar_chars = bar_array
            else:
                bar_chars = args.bar or hypr_config.get_value(
                    f"{prefix}_BAR", "▁▂▃▄▅▆▇█"
                )
        ...
        width = (
            ...
            else int(hypr_config.get_value(f"{prefix}_WIDTH", "0") or 0)
        )
        if not width:
        ...
        if args.stb is not None:
            ...
            standby_mode = hypr_config.get_value(f"{prefix}_STANDBY", "0")
