...
if [ ! -z "${setTheme}" ] && [ ! -z "${setWall}" ]; then
  ...
  if [[ "${theme_hashes[*]}" == *"${inwallHash}"* ]]; then
    send_ephemeral_notif "hypr-wallpaper-kon-error" -a "Awww wallpaper" -i "${WALLPAPER_THUMB_DIR}/${inwallHash}.sqre" -t 3000 "Error" "Hash matched in ${setTheme}"
    exit 0
  fi
  ...
  send_ephemeral_notif "hypr-wallpaper-kon" -a "Awww wallpaper" -i "${WALLPAPER_THUMB_DIR}/${inwallHash}.sqre" -t 2000 "Wallpaper set in ${setTheme}"
