...
hash_file="${XDG_RUNTIME_DIR:-/tmp}/wal-qutebrowser-hash"
...
if [ -f "$state_dir/staterc" ]; then
    color_mode=$(grep 'selected_color_mode=' "$state_dir/staterc" | sed 's/selected_color_mode=//;s/"//g')
...
cat > "$config_dir/qute-hypr-sync.py" << PYEOF
...
echo "✓ Qutebrowser sync: border_radius=${hypr_border}px, color_mode=${color_mode_name} (${color_mode})"
