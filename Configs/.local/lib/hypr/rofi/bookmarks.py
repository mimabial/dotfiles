...
class XDGPaths:
    def __init__(self):
        ...
        self.CACHE_DIR = os.path.join(self.xdg_cache, "hypr")
        self.CONFIG_DIR = os.path.join(self.xdg_config, "hypr")
        self.RUNTIME_DIR = os.path.join(self.xdg_runtime, "hypr")
        self.DATA_DIR = os.path.join(self.xdg_data, "hypr")
    ...
    def read_chromium_bookmarks(self, bookmarks_file):
        ...
        try:
            ...
            for item in (
                ...
                if "url" in item:
                    bookmarks.append(
                        {"title": item.get("name", item["url"]), "url": item["url"]}
                    )
