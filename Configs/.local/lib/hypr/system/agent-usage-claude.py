...
# Bump when parse_project_file changes shape or meaning: the rows it cached
# against a transcript's mtime are otherwise reused forever.
...
def parse_project_file(path: Path) -> dict[str, Any]:
  ...
  with path.open("r", encoding="utf-8", errors="replace") as handle:
    for line_number, line in enumerate(handle, 1):
      ...
      # Zero-token rows are kept: the scan claims their dedup key before it
      # discards them, so dropping them here would let a copy in another
      # file be counted.
      rows.append([
        unique_key,
...
def cached_project_files(projects_path: Path) -> list[dict[str, Any]]:
  ...
  # rglob order, not sorted: a message id duplicated across transcripts is
  # claimed by whichever file is visited first, and 335 of them here carry a
  # different sessionId in each copy. Sorting would silently move the numbers.
  for path in projects_path.rglob("*.jsonl") if projects_path.is_dir() else []:
    ...
    if not valid_cache_entry(entry, info):
      try:
        ...
        # retries it instead of inheriting a partial read.
  ...
  # Rebuilt from the current tree, so transcripts that went away are pruned.
  ...
  for entry in cached_project_files(projects_path):
    ...
    for row in entry["rows"]:
      try:
        ...
        # A cache entry is only as trustworthy as the file it came from.
        continue
...
def collect_limits(access_token: str, expires_at_ms: int) -> dict[str, Any]:
  ...
  # This window holds even for --force. It is not a reuse heuristic to
  # overrule: Anthropic rate-limits the endpoint, and a person leaning on the
  # refresh key is precisely what trips it. Nothing is lost — a press more
  # than PROBE_MIN_INTERVAL_SECONDS after the last one still probes fresh.
