...
monitor_aggregate_file() {
  printf '%s\n' "$(monitor_state_dir)/monitor-toggles.lua"
}
...
monitor_rebuild_toggles_unlocked() {
...

  {
    ...
    printf '%s\n\n' "-- Remove fragments from $(monitor_toggle_dir) or use the monitor helper commands."
    while IFS= read -r -d '' fragment; do
      printf '%s\n' "-- source-fragment: ${fragment}"
      cat "${fragment}"
      printf '\n'
    done < <(find "$(monitor_toggle_dir)" -maxdepth 1 -type f -name '*.lua' -print0 2>/dev/null | sort -z)
  } >"${tmp_file}"
