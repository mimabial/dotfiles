...
change needs edits in both this file and the TUI, this file is not carrying
enough.

Every `key` is validated against the running compositor by SchemaTest in
../tests/test_looknfeel.py. The Hyprland wiki is not the contract; the installed
build is. `decoration:glow:falloff` and `dwindle:pseudotile` are documented
upstream but absent here, which is exactly what that test catches.
...
def layout_rows(engine):
    return LAYOUT_ROWS.get(engine, [])


def query_keys():
    """Only rows backed by a real Hyprland option. Synthetic and pipeline rows
    carry an `id` instead and are driven separately."""
    return [row["key"] for section in SECTIONS for row in section["rows"] if "key" in row]


def listed_rows():
    """Rows whose options come from a helper rather than a literal list."""
    return [row for section in SECTIONS for row in section["rows"] if row.get("list")]
