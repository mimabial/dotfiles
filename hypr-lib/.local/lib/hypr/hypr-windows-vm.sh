...
install_windows() {
  ...
  hyprshell hypr-pkg-add.sh freerdp openbsd-netcat gum
  ...
  if [ -f "$HOME/HyDE/Configs/.config/menus/icons/windows.png" ]; then
    cp "$HOME/HyDE/Configs/.config/menus/icons/windows.png" "$HOME/.local/share/applications/icons/windows.png"
  fi
...
Exec=uwsm app -- hyprshell hypr-windows-vm.sh launch
...
EOF
  ...
  echo ""
  echo "To stop the VM: hyprshell hypr-windows-vm.sh stop"
...
launch_windows() {
  ...
  if [ ! -f "$COMPOSE_FILE" ]; then
    echo "Windows VM not configured. Please run: hyprshell hypr-windows-vm.sh install"
    exit 1
  fi
  ...
  if [ "$CONTAINER_STATUS" != "running" ]; then
    ...
    if ! docker-compose -f "$COMPOSE_FILE" up -d 2>&1; then
      echo "❌ Failed to start Windows VM!"
      echo "   Try checking: hyprshell hypr-windows-vm.sh status"
...
To stop: hyprshell hypr-windows-vm.sh stop"
  else
  ...
  if [ "$KEEP_ALIVE" = false ]; then
    ...
    echo "To stop it: hyprshell hypr-windows-vm.sh stop"
  fi
}
...
status_windows() {
  if [ ! -f "$COMPOSE_FILE" ]; then
    echo "Windows VM not configured."
    echo "To set up: hyprshell hypr-windows-vm.sh install"
    exit 1
  fi
  ...
  if [ -z "$CONTAINER_STATUS" ]; then
    ...
    echo "To start: hyprshell hypr-windows-vm.sh launch"
    ...
    echo "║                                    ║"
    echo "║  To connect:                       ║"
    echo "║    hyprshell hypr-windows-vm.sh launch       ║"
    echo "║                                    ║"
    echo "║  To stop:                          ║"
    echo "║    hyprshell hypr-windows-vm.sh stop         ║"
    echo "║                                    ║"
    echo "╚════════════════════════════════════╝"
  else
...
show_usage() {
  echo "Usage: hyprshell hypr-windows-vm.sh [command] [options]"
  echo ""
  echo "Commands:"
  ...
  echo ""
  echo "Examples:"
  echo "  hyprshell hypr-windows-vm.sh install           # Set up Windows VM for first time"
  echo "  hyprshell hypr-windows-vm.sh launch            # Connect to VM (auto-stop on exit)"
  echo "  hyprshell hypr-windows-vm.sh launch -k         # Connect to VM (keep running)"
  echo "  hyprshell hypr-windows-vm.sh stop              # Shut down the VM"
}
