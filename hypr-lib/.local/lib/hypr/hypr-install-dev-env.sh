...
if [[ -z "$1" ]]; then
  echo "Usage: hypr-install-dev-env <ruby|node|bun|go|laravel|symfony|php|python|elixir|phoenix|rust|java|ocaml|dotnet|clojure>" >&2
  exit 1
fi
...
ruby)
  ...
  hyprshell hypr-pkg-add.sh libyaml
...
symfony)
  ...
  install_php
  hyprshell hypr-pkg-add.sh symfony-cli
...
clojure)
  ...
  hyprshell hypr-pkg-add.sh rlwrap
